...
const setPageNumber = (function ()
{
	...
	const deletePhotoBts=contentElement.getElementsByClassName("deletePhotoBt");
	const deletePhotoForm=document.getElementById("deletePhotoForm");
	const deletePhotoInput=document.getElementById("deletePhotoID");
	const editPhotoBts=contentElement.getElementsByClassName("editPhotoBt");
	const editPhotoForm=document.getElementById("editPhotoForm");
	const editPhotoInput=document.getElementById("editPhotoID");
	const editPhotoPathInput=document.getElementById("editImagePathInput");
	...
	return function (pageIndex) // if there are n pages , than pageIndex can be : 0 , 1 , 2 , ...... , n-1
	{
		...
		for (let i = 0; i < 6; i++)
		{
			try
			{
				...
				editPhotoBts[i].onclick=function ()
				{
					editPhotoInput.value=this.imageID;
					editPhotoPathInput.value=this.path;
			...
			} catch (e)
			{
				...
				deletePhotoBts[i].imageID=undefined;
				deletePhotoBts[i].onclick=function ()
				{

				};
				editPhotoBts[i].imageID=undefined;
				editPhotoBts[i].path=undefined;
				editPhotoBts[i].onclick=function ()
				{
